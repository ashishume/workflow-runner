...
const operationLabel = computed(() => {
  ...
  switch (config.operation) {
    case 'uppercase': return `${config.field} → UPPERCASE`
    case 'lowercase': return `${config.field} → lowercase`
    case 'append': return `${config.field} + "${config.value}"`
    case 'prepend': return `"${config.value}" + ${config.field}`
    case 'multiply': return `${config.field} × ${config.value}`
    case 'add': return `${config.field} + ${config.value}`
    case 'replace': return `${config.field} = "${config.value}"`
    default: return config.operation
  }
})
</script>
