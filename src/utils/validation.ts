import type { WorkflowNode, WorkflowEdge, NodeType, WorkflowState } from '../types/workflow'
...
export function validateWorkflow(nodes: WorkflowNode[], edges: WorkflowEdge[]): ValidationResult {
  ...
  const otherNodes = nodes.filter(n => n.data.nodeType !== 'start' && n.data.nodeType !== 'end')
 ...
 */
export function validateImportedWorkflow(data: unknown): { valid: boolean; errors: string[] } {
  const errors: string[] = []
  ...
  if (!Array.isArray(workflow.nodes)) {
    ...
    workflow.nodes.forEach((node, index) => {
      ...
      // Validate node type
      const validTypes: NodeType[] = ['start', 'transform', 'condition', 'end']
