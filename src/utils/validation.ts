...
export function validateWorkflow(
  ...
  if (endNodes.length === 0) {
    warnings.push(
      "Workflow has no End node. Execution will stop after the last connected node."
    );
  }
  ...
  startNodes.forEach((node) => {
    ...
    if (!hasOutgoing) {
      warnings.push(
        `Start node "${node.data.label}" has no outgoing connections`
      );
    }
  });
  ...
  endNodes.forEach((node) => {
    ...
    if (!hasIncoming) {
      warnings.push(
        `End node "${node.data.label}" has no incoming connections`
      );
    }
  });
  ...
  const otherNodes = nodes.filter(
    (n) =>
      n.data.nodeType !== NodeType.START && n.data.nodeType !== NodeType.END
  );
  otherNodes.forEach((node) => {
    ...
    if (!hasIncoming && !hasOutgoing) {
      warnings.push(
        `Node "${node.data.label}" is not connected to the workflow`
      );
    } else if (!hasIncoming) {
  ...
  startNodes.forEach((node) => {
    try {
      ...
      if (!config.payload || typeof config.payload !== "object") {
        warnings.push(
          `Start node "${node.data.label}" has invalid payload configuration`
        );
      }
    } catch {
      warnings.push(
        `Start node "${node.data.label}" has invalid configuration`
      );
    }
  });
  ...
  if (!Array.isArray(workflow.nodes)) {
    ...
    workflow.nodes.forEach((node, index) => {
      ...
      if (node.type && !validTypes.includes(node.type as NodeType)) {
        errors.push(
          `Node "${node.id || index}" has invalid type "${node.type}"`
        );
      }
    });
  }
